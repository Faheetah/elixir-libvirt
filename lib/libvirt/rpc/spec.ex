defmodule Libvirt.RPC.Spec do
  ...
  def field(["char", name, _length]), do: {name, quote(do: String.t)}
  ...
  def field(["remote_typed_param", name]) do
    {name, quote(do: %{field: String.t(), value: String.t() | boolean() | integer()})}
  end
  ...
  def field(["remote_nwfilter_binding", name]),
    do: field(["remote_nonnull_nwfilter_binding", name])
